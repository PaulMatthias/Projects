...
int main(){
    ...
    //look for maximum time for one step and set it as t_intervall for the line
    for (int i=0; i<worker_times.size();i++){
      if(t_intervall<worker_times[i]){
	t_intervall=worker_times[i];
      }
    }
    cout<<"Timestep of intervall for this line production is "<<t_intervall<<endl;
 ...
 * *********************************************************************************************/
	...
	if ((list_of_workers[i].skill1==1) && list_of_workers[i].status=="free"){
      ...
      //Loop cells and send looking products to them every time intervall
      if (t%t_intervall==0){
	for (unsigned int i=0;i<list_of_workers.size();i++){
